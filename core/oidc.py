...
class CustomOIDCAuthenticationBackend(OIDCAuthenticationBackend):
    ...
    def create_user(self, claims):
        ...
        # Set user attributes from claims
    ...
    def get_username(self, claims):
        ...
        Priority order:
        1. preferred_username (Keycloak standard)
        2. email local part
...
def provider_logout(request):
    """
    Construct Keycloak logout URL for RP-initiated logout.

    This performs a full logout from Keycloak, not just the Django session.

    Args:
        request: Django request object

    Returns:
        Keycloak logout URL string
    """
    # Get the OIDC provider logout endpoint
    discovery_endpoint = getattr(settings, 'OIDC_OP_DISCOVERY_ENDPOINT', '')
    if not discovery_endpoint:
        logger.warning("OIDC_OP_DISCOVERY_ENDPOINT not configured, cannot construct logout URL")
        return None

    # Construct logout endpoint (standard OIDC endpoint path)
    # Discovery endpoint format: https://keycloak/realms/realm/.well-known/openid-configuration
    # Logout endpoint format: https://keycloak/realms/realm/protocol/openid-connect/logout
    ...
    # Get post logout redirect URI (where Keycloak sends user after logout)
    ...
    logger.debug(f"Constructed Keycloak logout URL: {logout_url}")
