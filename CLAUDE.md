...
- **Payload slimming**: MCP logbook responses (`search_logbook`, summary `recent_logs`) strip the web serializer's inlined document page manifests via `_slim_logbook_entry` — a 100-entry response was 1.39 MB otherwise. Keep MCP responses slim; agents get `{id, name}` document refs + `page_number`.
